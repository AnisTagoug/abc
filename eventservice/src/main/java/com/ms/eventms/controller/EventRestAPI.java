...
public class EventRestAPI {
    ...
    @PutMapping("/incrementNbrp/{idEvent}")
    public ResponseEntity<Event> incrementNbrp(@PathVariable("idEvent") Long idEvent) {
        Event event = eventRepository.findById(idEvent).orElse(null);
        if (event == null) {
            return new ResponseEntity<Event>(null);
        }
        event.setNbrp(event.getNbrp() + 1); // Incrémentation de nbrp
